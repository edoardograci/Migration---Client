...
interface WebPComparisonModalProps {
    ...
    targetId: string;
    targetName: string;
    targetType: 'designers' | 'moodboard';
    onAccept: (result: any) => void;
}

export function WebPComparisonModal({ isOpen, onClose, imageUrl, targetId, targetName, targetType, onAccept }: WebPComparisonModalProps) {
    ...
    const updateLocalDesigner = useDesignerStore(state => state.updateLocalDesigner);
    // const updateLocalProduct = useMoodboardStore(state => state.updateLocalProduct); // Will implement if needed
    ...
    const convert = async (strategy: string) => {
        setStep('converting');
        try {
            // Use targetName for slug if available, fallback to targetId
            const nameForSlug = targetType === 'designers' ? targetName : targetId;
            const slug = nameForSlug.toLowerCase()
                .replace(/[^a-z0-9]+/g, '-')
                .replace(/(^-|-$)+/g, '');

            const storageKey = targetType === 'designers'
                ? `studios/${slug}.webp`
                : `moodboard/${slug}/0.webp`;

            const res = await imageApi.convert(imageUrl, strategy, false, storageKey, targetType === 'moodboard');
            setResult(res);
            setStep('review');
        } catch (err: any) {
    ...
    const handleAccept = () => {
        if (targetType === 'designers' && result) {
            const nameForSlug = targetType === 'designers' ? targetName : targetId;
            const slug = nameForSlug.toLowerCase()
                .replace(/[^a-z0-9]+/g, '-')
                .replace(/(^-|-$)+/g, '');

            updateLocalDesigner(targetId, {
                status: {
                    migrated: true,
                ...
                coverUrl: `studios/${slug}.webp` // Point to optimized version
            });
        }
        onAccept(result);
    };
                ...
                <div className="p-4 border-t flex justify-between bg-muted/5">
                    ...
                    {step === 'review' && (
                        <div className="flex gap-2">
                            ...
                            >
                                <Check className="w-4 h-4" /> Accept & Save Local
                            </Button>
                        </div>
                    )}
                </div>
